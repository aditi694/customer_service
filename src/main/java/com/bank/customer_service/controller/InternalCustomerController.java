...
public class InternalCustomerController {
    ...
    public CustomerDetailResponse getCustomerDetail(@PathVariable UUID customerId) {
        return service.getCustomerDetails(customerId);
    }

    @GetMapping("/{customerId}/summary")
    public CustomerSummary getCustomerSummary(@PathVariable UUID customerId) {
    ...
    public String getCustomerContact(@PathVariable UUID customerId) {
        Customer c = customerRepo.findById(customerId)
                .orElseThrow(BusinessException::customerNotFound);
        return c.getPhone();
    }
    ...
    public String getIfscByAccount(@PathVariable String accountNumber) {
        Customer c = customerRepo.findByAccountNumber(accountNumber)
                .orElseThrow(() -> BusinessException.notFound("Customer not found for account"));
        return c.getIfscCode();
    }

    @GetMapping("/bank-branch/{ifscCode}")
    public BankBranchResponse getBankBranch(@PathVariable String ifscCode) {
        BankBranch branch = bankBranchRepo.findByIfscCode(ifscCode)
                .orElseThrow(() -> BusinessException.notFound("Bank branch not found"));
